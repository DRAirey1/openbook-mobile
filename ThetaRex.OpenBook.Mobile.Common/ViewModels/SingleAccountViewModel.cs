...
namespace ThetaRex.OpenBook.Mobile.Common.ViewModels
{
    ...
    public class SingleAccountViewModel : ScenarioViewModel
    {
        ...
        private static readonly Dictionary<Scenario, string> ProposedOrderDictionary = new Dictionary<Scenario, string>()
        {
            ...
            { Scenario.BuyDerivative, "ThetaRex.OpenBook.Mobile.Common.Data.Buy Derivative.json" },
        ...
        public SingleAccountViewModel(IRepository repository, IStringLocalizer<SingleAccountViewModel> stringLocalizer)
        {
            ...
            this.Items.Add(
                ...
                {
                    ActiveHandler = this.DeleteProposedOrderAsync,
                    ActiveLabel = this.stringLocalizer["DerivativeActiveLabel"],
                    Command = new Command<Scenario>((s) => this.RouteCommand(s)),
                    CommandParameter = Scenario.BuyDerivative,
                    Description = this.stringLocalizer["DerivativeDescription"],
                    InactiveHandler = this.BuyProposedOrdersAsync,
                    InactiveLabel = this.stringLocalizer["DerivativeInactiveLabel"],
                    Scenario = Scenario.BuyDerivative,
                });
        ...
        private async Task BuyProposedOrdersAsync(ScenarioItemViewModel scenarioItemViewModel)
        {
            ...
            List<ProposedOrderRequest> proposedOrderRequest = ResourceHelper.ReadEmbeddedFile<List<ProposedOrderRequest>>(
                Assembly.GetExecutingAssembly(),
                SingleAccountViewModel.ProposedOrderDictionary[(Scenario)scenarioItemViewModel.CommandParameter]);
            scenarioItemViewModel.Data = await this.repository.AddProposedOrdersAsync(proposedOrderRequest).ConfigureAwait(true);
